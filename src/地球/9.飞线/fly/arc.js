...
function flyArc(lon1, lat1, lon2, lat2) {
  ...
  var startSphereCoord = new THREE.Vector3(
    sphereCoord1.x,
    sphereCoord1.y,
    sphereCoord1.z
  );
  ...
  var endSphereCoord = new THREE.Vector3(
    sphereCoord2.x,
    sphereCoord2.y,
    sphereCoord2.z
  );
...
function _3Dto2D(startSphere, endSphere) {
  ...
  var quaternion3D_XOY = new THREE.Quaternion().setFromUnitVectors(
    normal,
    xoyNormal
  );
   ...
   */
  var quaternionInverse = quaternion3D_XOY
    .clone()
    .invert()
